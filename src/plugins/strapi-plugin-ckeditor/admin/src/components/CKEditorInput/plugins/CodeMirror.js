...
export class CodeMirror extends Plugin {
    ...
    init() {
        ...
        // Get Config & Lang
        var config = editor.tools?.extend(defaultConfig, editor.config.codemirror || {}, true),
        ...
        if (requirePresent) {
            ...
            var location = editor.getUrl(editor.plugins.getPath("codemirror") + "js/");
        ...
        if (editor.elementMode === editor.ELEMENT_MODE_INLINE || editor.plugins.sourcedialog) {

            // Override Source Dialog
            editor.dialog.add("sourcedialog", function (editor) {
                var sizeDialog = editor.document.getWindow().getViewPaneSize(),
                ...
                return {
                    ...
                    minWidth: minWidth,
                    minHeight: minHeight,
                    resizable: editor.DIALOG_RESIZE_BOTH,
                    onLoad: function () {
                        this.on("resize",
                            function (event) {
                    ...
                    onShow: function (event) {
                        ...
                        if (config.autoLoadCodeMirror) {
                            ...
                            if (!IsStyleSheetAlreadyLoaded(rootPath + "css/codemirror.min.css")) {
                                editor.document.appendStyleSheet(rootPath + "css/codemirror.min.css");
                            }

                            if (config.theme.length &&
                                config.theme != "default" &&
                                !IsStyleSheetAlreadyLoaded(rootPath + "theme/" + config.theme + ".css")) {
                                editor.document.appendStyleSheet(rootPath + "theme/" + config.theme + ".css");
                            }
                            if (requirePresent) {
                                ...
                                if (typeof (CodeMirror) == "undefined") {
                                    ...
                                    editor.scriptLoader.load(rootPath + "js/codemirror.min.js",
                                        function () {

                                            editor.scriptLoader.load(getCodeMirrorScripts(),
                                                function () {
                                                    loadCodeMirrorInline(editor, textArea, event.sender);
                                                });
                                        });
                    ...
                    onOk: (function () {
                        ...
                        return function () {
                            ...
                            editor.env.ie ? editor.tools.setTimeout(setData, 0, this, newData) : setData.call(this, newData);
        ...
        var sourcearea = editor.plugins.sourcearea;

        // check if sourcearea plugin is overrriden
        if (!sourcearea.commands.searchCode) {

            editor.plugins.sourcearea.commands = {
                source: {
                    modes: {
                        wysiwyg: 1,
                        source: 1
                    ...
                    exec: function (editorInstance) {
                        ...
                        editorInstance.getCommand("source").setState(editor.TRISTATE_DISABLED);
                ...
                autoCompleteToggle: {
                    ...
                    readOnly: 1,
                    exec: function (editorInstance) {
                        if (this.state == editor.TRISTATE_ON) {
                            window["codemirror_" + editorInstance.id].setOption("autoCloseTags", false);
                        } else if (this.state == editor.TRISTATE_OFF) {
        ...
        function loadCodeMirror(editor) {
            ...
            textarea.setStyles(
                editor.tools?.extend({
                    ...
                    width: editor.env.ie7Compat ? "99%" : "100%",
                    height: "100%",
                    resize: "none",
                    outline: "none",
                    'text-align': "left"
                },
                    editor.tools.cssVendorPrefix("tab-size", editor.config.sourceAreaTabSize || 4)));
            var ariaLabel = [editor.lang.editor, editor.name].join(",");
            textarea.setAttributes({
                dir: "ltr",
                tabIndex: editor.env.webkit ? -1 : editor.tabIndex,
                'role': "textbox",
        ...
        if (config.enableCodeFormatting) {
            ...
            if (editor.ui.addButton) {
                if (config.showFormatButton || config.showCommentButton || config.showUncommentButton || config.showSearchButton) {
                    editor.ui.add("-", editor.UI_SEPARATOR, { toolbar: "mode,30" });
                }
        ...
        editor.on("mode", function () {
            editor.getCommand("source").setState(editor.mode === "source" ? editor.TRISTATE_ON : editor.TRISTATE_OFF);

            if (editor.mode === "source") {
                if ("autoCompleteToggle" in editor.commands) {
                    editor.getCommand("autoCompleteToggle").setState(window["codemirror_" + editor.id].config.autoCloseTags ? editor.TRISTATE_ON : editor.TRISTATE_OFF);
                }

                if (editor.plugins.textselection && textRange && !editor.config.fullPage) {

                    //textRange.element = new editor.dom.element(editor._.editable.$);
                    //textRange.select();
        ...
        var sourceEditable = editor.tools.createClass({
            base: editor.editable,
            proto: {
            ...
            commands: {
                source: {
                    ...
                    exec: function (editor) {
                        ...
                        editor.getCommand("source").setState(editor.TRISTATE_DISABLED);
        ...
        function IsStyleSheetAlreadyLoaded(href) {
            return editor.document.getHead().findOne('link[href="' + href + '"]') != null;
        }
    }
}
